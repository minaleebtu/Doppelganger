...
def getCombinedProbs(splitData, prob_per_author):
    ...
    for i in range(len(allAuthors)):
        for j in range(i + 1, len(allAuthors)):
            ...
            # else:
            #     avg_prob[avg] = result
    return np.fromiter(avg_prob, dtype=float)
...
predictions_20_20_hat = cross_val_predict(clf, X_20_20, y_20_20, cv=kfold, method='predict_proba', groups=y_20_20)
predictions_40_20_hat = cross_val_predict(clf, X_40_20, y_40_20, cv=kfold, method='predict_proba', groups=y_40_20)
predictions_60_20_hat = cross_val_predict(clf, X_60_20, y_60_20, cv=kfold, method='predict_proba', groups=y_60_20)
predictions_60_10_hat = cross_val_predict(clf, X_60_10, y_60_10, cv=kfold, method='predict_proba', groups=y_60_10)
predictions_60_30_hat = cross_val_predict(clf, X_60_30, y_60_30, cv=kfold, method='predict_proba', groups=y_60_30)
...
x_axis = np.arange(len(numOfP) + len(numOfCommPerP))
plot_list = [numOfP, numOfCommPerP]
...
plot_list_flat = [item for elem in plot_list for item in elem]
for i, v in enumerate(plot_list_flat):
